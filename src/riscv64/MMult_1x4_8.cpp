...
void AddDot1x4( int k, float *a, float *b, int n, float *c)
{
  /* So, this routine computes four elements of C:
    ...
    for ( p=0; p<k; p+=4 ){
        ...
        c_02_reg += a_0p_reg * *bp1_pntr++;
        c_03_reg += a_0p_reg * *bp1_pntr++;

        a_0p_reg = A( 0, p + 2);
        bp2_pntr = &B( p + 2, 0 );
        c_00_reg += a_0p_reg * *bp2_pntr++;
        c_01_reg += a_0p_reg * *bp2_pntr++;
        c_02_reg += a_0p_reg * *bp2_pntr++;
        c_03_reg += a_0p_reg * *bp2_pntr++;

        a_0p_reg = A( 0, p + 3);
        bp3_pntr = &B( p + 3, 0 );
        c_00_reg += a_0p_reg * *bp3_pntr++;
        c_01_reg += a_0p_reg * *bp3_pntr++;
        c_02_reg += a_0p_reg * *bp3_pntr++;
